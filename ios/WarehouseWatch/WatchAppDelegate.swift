...
/// relaunches the app to finish downloads that completed while it was suspended
final class WatchAppDelegate: NSObject, WKApplicationDelegate {
    func handle(_ backgroundTasks: Set<WKRefreshBackgroundTask>) {
        for task in backgroundTasks {
            if let urlTask = task as? WKURLSessionRefreshBackgroundTask {
                WatchBackgroundDownloader.shared.reconnect(sessionIdentifier: urlTask.sessionIdentifier) {
